...
let BackstageComponent = class BackstageComponent {
    constructor(aa) {
        this.aa = aa;
    }
    ngOnInit() {
        ...
        let user = document.getElementById('user'); /*获取用户名*/
        let identity = document.getElementById('identity'); /*获取身份*/
        user.innerText = response.data.manager.name; /*给用户名赋值*/
        identity.innerText = response.data.role.name; /*给身份赋值*/
        console.log(window.innerHeight);
        let list = document.getElementById('list'); /*左侧列表*/
    ...
    ngAfterViewInit() {
        ...
        let page = list.getElementsByTagName('li');
        ...
        let list_page = document.getElementsByClassName('list_page')[0]; /*获取总的ul列表标签*/
        let surface_list = document.getElementsByClassName('surface_list'); /*获取表面列表的li标签*/
        let icon = [];
        for (let u = 0; u < surface_list.length; u++) {
        ...
        let list_page_Child = list_page.children; /*获取ul列表的所有子元素*/
        console.log(list_page_Child);
        let li = [];
        let ul = [];
        for (let r = 0; r < list_page_Child.length; r++) {
            if (r % 2 === 0) {
...
BackstageComponent.ctorParameters = () => [
...
let EditComponent = class EditComponent {
    constructor(http, https, router, aa) {
        this.http = http;
        this.https = https;
        this.router = router;
        this.aa = aa;
    ...
    ngOnInit() {
        ...
        this.editor.create();
        console.log(this.aa);
        this.aa.queryParamMap.subscribe((data) => {
            console.log(data.params.id);
            this.id = data.params.id;
        });
        console.log(this.id);
        let api = ' /a/article/' + this.id;
        this.http.get(api).subscribe((response) => {
            console.log(response);
            this.title = response.data.article.title;
            this.type.value = response.data.article.type;
            if (response.data.article.content != '' && response.data.article.content != null && response.data.article.content != undefined) {
                this.explain = response.data.article.content;
            }
            else {
                this.explain = '请输入内容';
            }
            console.log(this.explain);
            this.editor.txt.text(this.explain);
            console.log(this.editor.txt.html());
            this.jumplink = response.data.article.url;
            let s = document.getElementById('imj'); /*渲染图片*/
            s.src = response.data.article.img;
            this.image = response.data.article.img;
            if (response.data.article.type == 3) {
                this.dt.value = response.data.article.industry;
            }
        });
    }
    // 表单绑定change事件
    ...
    online_immediately() {
        ...
        this.put(api, httpOptions, data).subscribe((response) => {
            console.log(response);
            if (response.code == 0) {
                this.router.navigate(['/Backstage2/Article'], { queryParams: {
                        page: 1,
